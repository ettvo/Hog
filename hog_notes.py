...
"""
This is a list of the functions that are implemented in each problem.
Scroll down to see specific notes and problems while implementing each.
Asterisks indicate the problematic ones.

Phase 1: Game
1) roll_dice
2) piggy_points, no using for loops or [ ]
3) take_turn
4) * more_boar, the most problematic implementation for later questions
5) * play

Phase 2: Commentary
6) * play (update for commentary)
7) * announce_highest
8) * make_averaged
9) * max_scoring_num_rolls
10) * piggypoints_strategy
11) 
12) Challenge: 
...
def more_boar(player_score, opponent_score):
    ...
    digit_place_difference = len(str(opponent_score)) - len(str(player_score))

    if len(str(player_score)) == 1: 
        player_first_digit = 0
        player_second_digit = player_score
    else:
        player_first_digit = player_score // pow(10, len(str(player_score)) - 1)
        player_second_digit = player_score / (pow(10, len(str(player_score)) - 2)) % 10

    if len(str(opponent_score)) == 1:
        opponent_first_digit = 0
        opponent_second_digit = opponent_score
    else:
        opponent_first_digit = opponent_score // pow(10, len(str(opponent_score)) - 1)
        opponent_second_digit = opponent_score / pow(10, len(str(opponent_score)) - 2) % 10

    if player_first_digit < opponent_first_digit and player_second_digit < opponent_second_digit:
        return True
    else:
        return False
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1, prev_score=last_score, max_gain=running_high):
        max_gain_change = False

        if not who:
            if score0 > prev_score:
                if max_gain < (score0 - prev_score):
                    max_gain = score0 - prev_score
                    max_gain_change = True
                    prev_score = score0
        else:
            if score1 > prev_score:
                if max_gain < (score1 - prev_score):
                    max_gain = score1 - prev_score
                    max_gain_change = True
                    prev_score = score1

        if max_gain_change:
            print("Player", int(who), "has reached a new maximum point gain.", max_gain, "point(s)!")
        
        if not who:
            return announce_highest(who, score0, max_gain)
        else:
            return announce_highest(who, score1, max_gain)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for x in range(10):
        avg = make_averaged(roll_dice, trials_count)(x + 1, dice)
        if avg > maximum_avg:
            maximum_avg = avg
            dice_num = x + 1
